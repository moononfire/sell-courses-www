...
export function NewSeriesForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const body = {
      ...
      priceCents: Math.round(parseFloat(fd.get("price") as string) * 100),
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      {[
        ...
        { name: "price", label: "Cena (PLN)", type: "number", required: true },
        { name: "thumbnailUrl", label: "URL miniatury", type: "url", required: false },
      ].map((f) => (
      ...
      >
        {loading ? "Zapisywanie…" : "Utwórz serię"}
      </button>
    </form>
  );
}
